...
class CrisisEnvironment:
    ...
    def _advance_dynamics(self) -> Tuple[List[str], Dict[str, float]]:
        ...
        for threat in active_threats:
            ...
            # PATCH 2: Lower containment so threats actually impact → rescue is scoreable
            if truth["tti"] > 0 and self._threat_mitigation[tid] > 0.80 and self._np_rng.random_sample() < 0.20:
    ...
    def _build_observation(self, alerts: List[str]) -> CrisisObservation:
        ...
        enriched_alerts = list(alerts)  # hint removed: agents should reason, not be told
    ...
    def _generate_threats(self, threat_count: int) -> List[ThreatInfo]:
        ...
        for i, (threat_type, zone, location, base_pop) in enumerate(chosen, start=1):
            ...
            # FIX: Guarantee 1st threat has TTI=6 — ensures rescue is always possible
            if i == 1:
                tti = 6
            else:
